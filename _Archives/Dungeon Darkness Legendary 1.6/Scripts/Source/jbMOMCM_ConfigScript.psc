...
if page == "Basic Config"
	...
	AddToggleOptionST("OPTION_disableAI", "Disable AI during fadeout?",Options.disableAI)
	...
	AddToggleOptionST("OPTION_enableFireLighter", "Enable lighting of sconce fires",Options.enableFireLighter)
...
state OPTION_disableAI
    event OnSelectST()
		Options.Toggle_disableAI()
		SetToggleOptionValueST(Options.disableAI)
    endEvent

    event OnDefaultST()
        SetToggleOptionValueST(true)
		Options.disableAI = true
    endEvent

    event OnHighlightST()
        SetInfoText("Disable dungeon NPCs AI during fadeout.  Only effective if Fadeout is enabled.")
    endEvent
endState
...
state OPTION_enableFireLighter
    ...
    event OnHighlightST()
        SetInfoText("Enable activators for sconces - light and extinguish fires in sconces.")
    endEvent
endState
