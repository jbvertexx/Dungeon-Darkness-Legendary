...
function loadVars(jbMOMainQS akMainQS, Cell akCell, int akRunType, bool akDisableAI)
 
    ;Store our passed-in parameters to member variables
	theRunType = akRunType
	doDisableAI = akDisableAI
	theMainQS = akMainQS
...
Event onScanNPCs()
	if thread_queued
		...
		if theActor
			...
			elseif theRunType == 1
				;2nd scan to re-enable AI
				jbUtils.DebugTrace("Actor "+theIndexVal+": "+theActor+" AI Enabled")
